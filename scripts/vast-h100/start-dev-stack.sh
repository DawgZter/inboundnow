...
tmux new-window -t "$SESSION" -n moss "cd '$ROOT_DIR' && MOSS_RUNTIME_PORT=$MOSS_PORT npm run dev:moss-runtime"
tmux new-window -t "$SESSION" -n agent "cd '$ROOT_DIR' && TOKEN_SERVER_URL=http://127.0.0.1:$TOKEN_PORT LIVEKIT_ROOM=$ROOM AGENT_TRANSPORT=livekit LLM_PROVIDER=qwen-openai-local LLM_BASE_URL=http://127.0.0.1:$LLM_PORT/v1 LLM_MODEL=$LLM_MODEL_NAME MOSS_PROVIDER=local-runtime-client MOSS_RUNTIME_URL=http://127.0.0.1:$MOSS_PORT npm run dev:agent:livekit"
