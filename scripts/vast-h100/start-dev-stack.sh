...
tmux new-window -t "$SESSION" -n agent "cd '$ROOT_DIR' && TOKEN_SERVER_URL=http://127.0.0.1:$TOKEN_PORT LIVEKIT_ROOM=$ROOM AGENT_TRANSPORT=livekit AGENT_PLANNER=local-llm LLM_PROVIDER=qwen-openai-local LLM_BASE_URL=http://127.0.0.1:$LLM_PORT/v1 LLM_MODEL=$LLM_MODEL_NAME MOSS_PROVIDER=local-runtime-client MOSS_RUNTIME_URL=http://127.0.0.1:$MOSS_PORT TTS_PROVIDER=$TTS_PROVIDER_NAME TTS_BASE_URL=http://127.0.0.1:$TTS_PORT TTS_DTYPE=${TTS_DTYPE:-bfloat16} TTS_QUANTIZATION=${TTS_QUANTIZATION:-llm-int8} TTS_CACHE_DIR=${TTS_CACHE_DIR:-artifacts/cache/tts} npm run dev:agent:livekit"
...
echo "  ssh -p <vast-ssh-port> root@<vast-host> -L $LAB_PORT:127.0.0.1:$LAB_PORT -L $TOKEN_PORT:127.0.0.1:$TOKEN_PORT -L 7880:127.0.0.1:7880 -L $LLM_PORT:127.0.0.1:$LLM_PORT -L $MOSS_PORT:127.0.0.1:$MOSS_PORT -L $TTS_PORT:127.0.0.1:$TTS_PORT"
echo "Enable VibeVoice tmux pane with: ENABLE_TTS_RUNTIME=1 bash scripts/vast-h100/start-dev-stack.sh"
