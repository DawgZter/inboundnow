...
tmux new-window -t "$SESSION" -n agent "cd '$ROOT_DIR' && TOKEN_SERVER_URL=http://127.0.0.1:$TOKEN_PORT LIVEKIT_ROOM=$ROOM AGENT_TRANSPORT=livekit AGENT_PLANNER=local-llm LLM_PROVIDER=qwen-openai-local LLM_BASE_URL=http://127.0.0.1:$LLM_PORT/v1 LLM_MODEL=$LLM_MODEL_NAME ASR_PROVIDER=$ASR_PROVIDER_NAME ASR_BASE_URL=http://127.0.0.1:$ASR_PORT MOSS_PROVIDER=local-runtime-client MOSS_RUNTIME_URL=http://127.0.0.1:$MOSS_PORT TTS_PROVIDER=$TTS_PROVIDER_NAME TTS_BASE_URL=http://127.0.0.1:$TTS_PORT TTS_DTYPE=${TTS_DTYPE:-bfloat16} TTS_QUANTIZATION=${TTS_QUANTIZATION:-llm-int8} TTS_CACHE_DIR=${TTS_CACHE_DIR:-artifacts/cache/tts} npm run dev:agent:livekit"
