...
try {
  ...
  await page.waitForFunction(() => Boolean(window.OpenClickyWeb && document.querySelector(".ocw-cal-frame")), null, { timeout: 10_000 });
  ...
  await page.evaluate(() => window.OpenClickyWeb.dispatch("confirmBooking"));
  await page.waitForFunction(() => window.OpenClickyWeb.events().some((event) => event.type === "bookingPromptShown"), null, { timeout: 5000 });
  ...
  await page.waitForFunction(() => window.OpenClickyWeb.events().some((event) => event.type === "openCalDeferred"), null, { timeout: 5000 });
  ...
  await page.waitForFunction(() => {
    const prompt = document.querySelector(".ocw-booking-prompt");
    return prompt?.classList.contains("is-open") && window.OpenClickyWeb.events().filter((event) => event.type === "bookingPromptShown").length >= 2;
  }, null, { timeout: 5000 });
