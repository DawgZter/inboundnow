...
| Browser ASR UI state | verified | `npm run smoke:browser:asr-ui` | Browser widget shows current ASR boundary copy, listening/no-audio states, final transcript fallback, preserved voice profile, and streamed speech event after an ASR final transcript. |
...
- `npm run check` passed on June 7, 2026, including 47 tests, adapter/planner/Moss/ASR/TTS/voice smokes, the browser Cal gate smoke, browser ASR UI smoke, and Miso LoRA manifest validation.
