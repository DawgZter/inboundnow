...
- `npm run smoke:local` passed at `artifacts/smoke/2026-06-07T13-15-39-821Z/result.json`, including `streamedSpeechEvents: true`.
- `npm run smoke:livekit` passed with `bridgeDisabled: true` at `artifacts/smoke/livekit-2026-06-07T13-15-40-579Z/result.json`, including `dataChannelSpeechStream: true`.
...
- `npm run smoke:asr:livekit` passed at `artifacts/smoke/livekit-asr-local-2026-06-07T13-14-51-130Z/result.json`, proving a synthetic LiveKit microphone track reached the worker `AudioStream(track)` buffer, encoded a WAV, hit the fake localhost Parakeet endpoint, emitted `agent.asr.final` with `source: livekit-audio-turn`, and drove answer/action. This is not real browser microphone or H100 Parakeet proof.
- `npm run smoke:browser:asr-ui` passed at `artifacts/smoke/browser-asr-ui-2026-06-07T13-20-16-822Z/result.json`, proving the browser ASR chips/statuses update for listening, no-audio, and final transcript fallback; transcript display includes `Heard: ...`; the selected voice profile survives ASR final and answer events; and streamed speech starts.
...
- `npm run smoke:browser:cal-gate` passed at `artifacts/smoke/browser-cal-gate-2026-06-07T13-15-28-609Z/result.json`, proving the injected browser widget keeps the Cal iframe `src` empty on load, blocks direct `confirmBooking`, defers `openCal`, re-prompts after a dismissed prompt, and loads Cal only after clicking `Yes, open Cal`.
